...
from InfoPanel.info_services import WeatherService, SunService
...
class WeatherMode(Mode):
    ...
    def apply(self, last_mode: Mode):
        ...
        if (self._last_update is not None and self._last_update != datetime.now().date()) or not isinstance(last_mode,
                                                                                                            WeatherMode):
...
class SunMode(Mode):
    ...
    def apply(self, last_mode: Mode):
        ...
        now = time.time()
        if (self._last_update is not None and self._last_update + self._change_period < now) or not isinstance(
                last_mode, SunMode):
